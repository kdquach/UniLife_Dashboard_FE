...
import { Card, Form, Input, TimePicker, Button, Tag, Empty, message, Tabs, Table, Select, Badge } from "antd";
import dayjs from "dayjs";
import GIcon from "@/components/GIcon";
import { useAuthStore } from "@/store/useAuthStore";
import { getCanteenById, createCanteen, getAllCanteens, updateCanteen } from "@/services/canteen.service";
...
export default function CanteenManagement() {
    ...
    const handleSubmit = useCallback(
        async (values) => {
            ...
            const payload = {
                ...
                openingTime: values.openingTime ? values.openingTime.format(timeFormat) : undefined,
                closingTime: values.closingTime ? values.closingTime.format(timeFormat) : undefined,
            };
            ...
            try {
                ...
                message.error(
                    error?.response?.data?.message || "Không thể lưu thông tin căng tin. Vui lòng thử lại.",
                );
            } finally {
                setSubmitting(false);
            }
    ...
    const managerColumns = [
        ...
        },
        {
            title: "Giờ mở cửa",
            key: "time",
            render: (_, record) => `${record.openingTime || ""} - ${record.closingTime || ""}`,
        },
        {
            title: "Trạng thái",
            dataIndex: "status",
    ...
    const handleAdminApprove = useCallback(
        async (id) => {
            try {
                await updateCanteen(id, { status: "active" });
                message.success("Đã duyệt căng tin, chuyển sang trạng thái hoạt động");
                await loadCanteen();
            } catch (error) {
                message.error(
                    error?.response?.data?.message || "Không thể duyệt căng tin. Vui lòng thử lại.",
                );
            }
        },
        [loadCanteen],
        ...
        return (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div>
                        <div style={{ fontSize: 14, color: "var(--text-muted)", marginBottom: 4 }}>Quản lý</div>
                        ...
                        },
                        {
                            ...
                            children: (
                                <Card className="surface-card" loading={loading}>
                                    ...
                                    >
                                        <div style={{ fontWeight: 600 }}>Danh sách căng tin chờ duyệt</div>
                                    </div>
                        ...
                        children: (
                            <>
                                <Card className="surface-card" loading={loading}>
                                    ...
                                    >
                                        <div style={{ fontWeight: 600 }}>Danh sách căng tin của bạn</div>
                                        {canteens.length === 0 && (
                                            <Button
                                                type="primary"
                                                icon={<GIcon name="add" />}
                                    ...
                                    {editMode && (
                                        ...
                                        >
                                            <Form.Item
                                                label="Tên căng tin"
                                                name="name"
                                                rules={[{ required: true, message: "Vui lòng nhập tên căng tin" }]}
                                            >
                                                <Input />
                                            </Form.Item>
                                ...
                                {canteens.filter((c) => c.status === "active").length === 0 ? (
                                    ...
                                    (() => {
                                        ...
                                        const parseTime = (timeStr, base) => {
                                            ...
                                            return base.hour(h || 0).minute(m || 0).second(0);
                                        };

                                        const now = dayjs();
                                        const todayBase = dayjs();
                                        const openToday = parseTime(canteen.openingTime, todayBase);
                                        const closeToday = parseTime(canteen.closingTime, todayBase);
                                        ...
                                        const timeRangeLabel = `${canteen.openingTime || "--:--"} - ${canteen.closingTime || "--:--"
                                            }`;
                                        ...
                                        const handleToggleOffDay = async (dateKey, isCurrentlyOff) => {
                                            console.log("🚀 ~ handleToggleOffDay ~ isCurrentlyOff:", isCurrentlyOff)
                                            console.log("🚀 ~ handleToggleOffDay ~ dateKey:", dateKey)
                                            try {
                                                ...
                                                console.log("🚀 ~ handleToggleOffDay ~ nextOffDates:", nextOffDates)
                                                ...
                                                >
                                                    <div>
                                                        ...
                                                        <div style={{ color: "var(--text-muted)", fontSize: 13 }}>
                                                            {canteen.name}
                                                            {campusName && campusName !== "-" && ` • ${campusName}`}
                                                        </div>
                                                    </div>
                                                                    ...
                                                                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                                                        ...
                                                                        {!isToday && (
                                                                            <Button
                                                                                size="small"
                                                                                onClick={() =>
                                                                                    handleToggleOffDay(d.key, isCurrentlyOff)
                                                                                }
                                                                            >
